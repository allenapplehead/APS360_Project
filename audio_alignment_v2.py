...
from pop2piano_dsp import normalize, get_stereo
...
def align_song_piano(
    ...
    song_audio = sample.raw_song_audio
    ...
    try:
        if dry_run:
            print("write audio files: ", sample.temp_song_audio_path)
        else:
            sf.write(
                file=sample.temp_song_audio_path,
                data=song_pitch_shifted,
                samplerate=Fs,
                format="wav",
            )
    except:
        print("Fail : ", sample.temp_song_audio_path)

    try:
        if dry_run:
            print("write warped midi :", sample.piano_midi_path)
        else:
            midi_warped_pm.write(sample.piano_midi_path)

    except:
        midi_warped_pm._tick_scales = midi_pm._tick_scales
        try:
            if dry_run:
                print("write warped midi2 :", sample.piano_midi_path)
            else:
                midi_warped_pm.write(sample.piano_midi_path)

        except:
            print("ad-hoc failed midi : ", sample.piano_midi_path)
        print("ad-hoc midi : ", sample.piano_midi_path)
